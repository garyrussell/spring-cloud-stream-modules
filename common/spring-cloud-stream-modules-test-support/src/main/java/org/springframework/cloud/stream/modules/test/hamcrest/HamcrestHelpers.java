...
public class HamcrestHelpers {
	...
	public static Matcher<ObjectError> isFieldErrorWith(String argument0, String defaultMessage) {
		return new FieldValidationMatcher(argument0, defaultMessage);
	}

	public static Matcher<ObjectError> withNonEmptyArgument(String argument0) {
	...
	private static final class FieldValidationMatcher extends DiagnosingMatcher<ObjectError> {
		...
		protected boolean matches(Object item, Description mismatchDescription) {
			if (item instanceof FieldError) {
				...
				return fieldError.getArguments().length == 1
